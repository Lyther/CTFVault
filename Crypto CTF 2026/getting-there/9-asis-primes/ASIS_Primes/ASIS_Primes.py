...
def main():
    ...
    while True:
        ...
        if ans == "e":
            ...
            pr(border, "Please submit the primes p, q: ")
            inp = sc().decode().strip()
            try:
                ...
                if (
                    isPrime(_p)
                    and isPrime(_q)
                    and _pbytes.startswith(pinit)
                    and _qbytes.startswith(qinit)
                    and _pbytes.endswith(b"}")
                    and _qbytes.endswith(b"}")
                    and is_valid(_pbytes)
                    and is_valid(_qbytes)
                    and (9 * _p * _q).bit_length() == 2 * nbit
                ):
                    p, q = _p, _q
            except:
                pr(border, "The input you provided is not valid! Try again!!")
                nbit += 1
        elif ans == "q":
            die(border, "Quitting...")
        else:
