...
def init_db():
    conn = get_db()
    conn.execute(
        "create table if not exists users (username text primary key, password text not null, role text not null, full_name text not null, title text not null, team text not null)",
    )
    conn.commit()
    conn.close()
...
def register():
    ...
    if (
        not record.get("username")
        or not record.get("password")
...
def dashboard():
    ...
    conn = get_db()
    user = conn.execute(
        "select username, role, full_name, title, team from users where username = ?",
        (session["username"],),
    ).fetchone()
    conn.close()
    if not user:
...
def update_profile():
    ...
    current = conn.execute(
        "select username, password, role, full_name, title, team from users where username = ?",
        (session["username"],),
    ).fetchone()
    if not current:
        conn.close()
    ...
    record.update(incoming)
    if record.get("password") != current["password"] and not valid_password(
        record.get("password", ""),
    ):
        conn.close()
        return jsonify({"error": "Password does not meet policy."}), 400
...
def admin():
    ...
    return render_template(
        ...
        flag=os.getenv("FLAG", "CIT{test_flag}"),
    )
